## 更新履歴
2026.01.14   ダブルクリック動作に [画像とメタデータを並べて表示](#サムネイル)を追加。 \
2026.01.14   画像プレビューにselectボタン追加。
# ImageManager
Stable Diffusion WebUI や reforge で生成された画像の **ファイルを移動・コピー** したり、 **プロンプトを確認** するためのアプリです。\
WebUI のデフォルト設定で作成される **日付ごとのフォルダをまとめて管理** できます。\
Python 3.10.6で動作確認
> [!NOTE]
> ComfyUIで生成した画像は各プロンプトが抽出できないためGenerate InfoにJSON生データが表示されます。 \
> Infoでのフィルタは可能ですが、タグ選択やワイルドカード作成など、プロンプトを利用する機能は使えません。
...
> [!TIP]
> サブフォルダ間で移動したファイルもサムネイルから削除されます。 \
> フォルダを選択し直すか、移動先フォルダを開くと表示されます。

> [!TIP]
> 移動中にキャンセルするとサムネイルが正しく更新されません。 \
> フォルダを選択し直すか、アプリを再起動してください。
### 画像ファイルのコピー
...
### サムネイル
- **マウスオーバー** でフォルダパス表示
- **ダブルクリック** で画像ビューワーを表示 [画像表示ダイアログ](#画像表示ダイアログ) \
（[サムネイルダブルクリック時の動作](#サムネイルダブルクリック時の動作)で「画像とメタデータを並べて表示」に変更可能）
  - アニメーションwebpは **V** アイコンが表示され、ダブルクリックで再生
  - **Selectボタン**: サムネイルの選択状態を切り替えます。未選択時はグレー、選択時はオレンジ色で「✔」が表示されます。
  - **通常モード**: 画像を大きく表示します。
  - **画像とメタデータを並べて表示**: 左側に画像、右側にメタデータを表示します。
   <img width="593" height="413" alt="Image" src="https://github.com/user-attachments/assets/3681874c-197f-443a-99bd-a31c5015a6f4" />
...
![Image](https://github.com/user-attachments/assets/d4b88a28-d4da-41ca-8c2c-82b9a0316b06)
  - 置換機能：検索する文字列、置換する文字列を入力し、`一括置換` クリック
  - 各画像のコメント、プロンプトを編集可能
  - `ファイルへ出力` でtxtファイルで出力
...
## 設定
<img width="448" height="870" alt="Image" src="https://github.com/user-attachments/assets/96889631-92a3-46cb-9dbb-e3a6e455a9d9" /> 
...
### サムネイルダブルクリック時の動作
- **画像ビューワーを開く (デフォルト)**: 通常の画像ビューワーを開きます。
- **画像とメタデータを並べて表示**: 画像の横にメタデータを表示するウィンドウを開きます。 \
![Image](https://github.com/user-attachments/assets/3681874c-197f-443a-99bd-a31c5015a6f4)
